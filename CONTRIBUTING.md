...
It never pushes to `main`. The push and the pull request use a token for a GitHub App, read from the `FORMULA_APP_ID` and `FORMULA_APP_PRIVATE_KEY` secrets, because Actions is not permitted to create pull requests in this repository, and a pull request opened with `GITHUB_TOKEN` would start no CI, so `test` would never report. The job fails if either secret is missing. If an older release's formula pull request is still open, the job closes it as superseded, since both change the same lines and whichever merged second would conflict. [docs/branch-protection.md](docs/branch-protection.md) says how the App is set up.
