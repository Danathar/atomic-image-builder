...
That is the whole manual process, apart from opening and merging one pull request. `.github/workflows/update-homebrew-formula.yml` takes it from there on `release: published` — it checks out `main` (the release event checks out the tag, and the formula lives on `main`), runs `--update` for the released tag, verifies the result with `--check`, and pushes the one-line change to a `formula/<tag>` branch. Its run summary and a reminder issue titled "Open the Homebrew formula PR for <tag>" both carry a link that opens the pull request with its title filled in. Click it, create the pull request, and merge it once `test` passes. Users get the new version on their next `brew update && brew upgrade` after that.

It never pushes to `main`, which the ruleset refuses anyway. It needs no secrets: the branch push and the issue use the workflow's own `GITHUB_TOKEN`. It does not open the pull request itself, because Actions is not permitted to create pull requests in this repository, and a pull request opened with `GITHUB_TOKEN` would start no CI, so `test` would never report. A person opening it is what makes `test` run. [docs/branch-protection.md](docs/branch-protection.md) has the details.
