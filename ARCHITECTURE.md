...
| Workflow patchers (module level) | `patch_signing_step_block()` and `patch_workflow_steps()`, then `patch_workflow_signing_steps()`, the `workflow_key`/`workflow_block_key` line classifiers, `patch_cosign_compatibility()`, and `workflow_job_ranges()` with `ensure_workflow_job_env_entries()`. |
...
| Host package queries  | `lookup_host_packages`, `lookup_removable_host_packages`, `lookup_installed_host_packages`, `search_host_packages`, `refresh_package_metadata`, `dnf5_state_dir`                                          |
